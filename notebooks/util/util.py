...
def _paths_to_coefficient_matrices(
    ...
    V = sparse.lil_matrix((nnodes, npaths))
    E = sparse.lil_matrix((nedges, npaths))
...
class PathSelectionSolver(object):
    ...
    def _recompute_matrices_osqp(self, paths):
    ...
    def _recompute_matrices_piqp(self, paths):
        # Store the paths (for solution reconstruction)
        self.paths = copy.deepcopy(paths)
        # Compute the coefficient matrices
        V, E = _paths_to_coefficient_matrices(self.tug, paths)
        ...
        self.P = P
        # Compute the linear coefficient vector
        c = -V.T.dot(v).T - E.T.dot(e).T + self.alpha
        self.c = c
        # Compute the constraint matrix and the bound vectors
        self.x_lb = np.zeros((len(paths), ))
        self.x_ub = None
        # Build the matrices for the min vertex cover constraint
        if self.mvc is not None:
            A, l, u = _min_vertex_cover_constraints(self.tug, paths,
            ...
            self.G = -A
            self.h = -l
        return self.P, self.c, self.A, self.b, self.G, self.h, self.x_lb, self.x_ub
    ...
    def solve(self, paths, **settings):
        # Recompute the problem matrices and setup the solver
        if self.solver == 'osqp':
            self.mdl = osqp.OSQP()
            P, q, A, l, u = self._recompute_matrices_osqp(paths)
            self.mdl.setup(P=P, q=q, A=A, l=l, u=u, **settings)
            # Solve the problem
            sol = self.mdl.solve()
            # Extract a solution
            ...
            if 'verbose' in settings and settings['verbose'] > 0:
                self.mdl.settings.verbose = True
            # self.mdl.settings.compute_timings = True
            P, c, A, b, G, h, x_lb, x_ub = self._recompute_matrices_piqp(paths)
            self.mdl.setup(P, c, A, b, G, h, x_lb, x_ub)
            # Solve the problem
            status = self.mdl.solve()
            # Obtain a solution
            self.sol = self.mdl.result
        return self.sol
    ...
    def get_cover_duals_osqp(self):
        if self.mvc is not None:
            return self.sol.y[-len(self.tug.vs):]
        else:
            return None

    def get_cover_duals_piqp(self):
        if self.mvc is not None:
            return -self.sol.z
        else:
            return None

    def get_non_negativity_duals_osqp(self):
        return self.sol.y[:len(self.paths)]

    def get_non_negativity_duals_piqp(self):
        return self.sol.z_lb
...
def solve_path_selection_full(
    ...
    prb = PathSelectionSolver(tug, node_counts, arc_counts,
                alpha=alpha, min_vertex_cover=min_vertex_cover, solver=solver)
    ...
    if return_duals:
        if solver == 'osqp':
            res.append(prb.get_non_negativity_duals_osqp())
            if min_vertex_cover is not None:
                res.append(prb.get_cover_duals_osqp())
        else:
            res.append(prb.get_non_negativity_duals_piqp())
            if min_vertex_cover is not None:
                res.append(prb.get_cover_duals_piqp())
    return res
...
def trajectory_extraction_cg(
    ...
    for it in range(max_iter):
        ...
        if solver == 'osqp':
            ...
            sol = master.solve(paths, verbose=0)
        ...
        if min_vertex_cover is not None:
            if solver == 'osqp':
                cover_duals = master.get_cover_duals_osqp()
            else:
                cover_duals = master.get_cover_duals_piqp()
        else:
            cover_duals = None
